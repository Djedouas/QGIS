...
{
  return QObject::tr( "This algorithm checks the geometries contained in other polygonal geometries (all features from all layers mixed)." );
}

auto QgsGeometryCheckContainedAlgorithm::flags() const -> Qgis::ProcessingAlgorithmFlags
{
...
void QgsGeometryCheckContainedAlgorithm::initAlgorithm( const QVariantMap &configuration )
{
  ...
  addParameter( new QgsProcessingParameterMultipleLayers( QStringLiteral( "INPUTS" ), QObject::tr( "Input layers (must contain at least one polygon type)" ) ) );
